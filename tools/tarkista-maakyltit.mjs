 ...
 *      matkakirjachipin mitoissa (≥ 22 px; 44 px:n osuma-alueen
 *      hoitaa ::after-laajennos) — sekä 1280 px:n että 390 px:n
  ...
  for (const viewport of VIEWPORTIT) {
    ...
    const tulos = await sivu.evaluate(() => {
      ...
      for (const iso of isot) {
        ...
        // Kyltti on matkakirjachipin mitoissa (omistajan tarkennus
        // 10.8.2026 ilta) — chippi on ~25 px; raja vartioi vain
        // romahdusta. Osuma-alue on ::after-laajennettu.
        if (r.height < 22) viat.push(`${iso} ${maa.nimi}: korkeus ${r.height.toFixed(1)} px < 22 px`);
