...
/*
 * PYYNNÖN KOORDINAATIT PYÖRISTETÄÄN KUUTEEN DESIMAALIIN.
 *
 * Mitattu 18.9.2026 (sarake 75, 154 puuttuvan laatan erä): NCSS
 * vastasi HTTP 500:lla ja rungossa luki
 * `AssertionError: -15.016666666666666 should be <= -15.01666`.
 * Luku on `−15 − 4/240` täydellä liukulukutarkkuudella, ja THREDDS
 * kaatuu omaan sisäiseen vertailuunsa, kun pyydetty reuna osuu
 * ~1e−11:n päähän hilan koordinaatista. Kyse ei ole päivämäärärajasta
 * eikä meidän ikkunastamme vaan palvelimen pyöristyksestä.
 *
 * Kuusi desimaalia on 0,11 m päiväntasaajalla eli tuhannesosa
 * 15″-solusta: pyyntö pysyy samassa solussa, mutta ei enää koskaan
 * osu assertion reunalle. VAHIN-levennys tehdään ennen tätä, joten
 * ohut pala pysyy ohuena mutta ehjänä.
 */
...
export async function haeIkkuna({
  ...
  const rajat = (a, b) => {
    ...
    for (let i = 0; i < reunat.length - 1; i++) {
      ...
      /*
       * ITÄREUNA VEDETÄÄN PUOLI SOLUA SISÄÄN, jos se osuu 15°:n rajaan.
       *
       * Tämä on kahden mitatun vian yhteinen juuri (18.9.2026, 154
       * puuttuvan laatan erä). Kun pyydetty `east` on täsmälleen
       * lähdelaatan itäreuna, THREDDS ei leikkaa ikkunaa vaan joko
       *   - kaatuu omaan tarkistukseensa (HTTP 500,
       *     `AssertionError: -15.0166666666 should be <= -15.01666`,
       *     sarake 75), tai
       *   - palauttaa KOKO TIEDOSTON LEVEYDEN pyydetyn ikkunan sijaan
       *     (86 919 saraketta 519:n sijaan, sarake 166) — jolloin pala
       *     liimautuu hilassa maailman toiselle puolelle.
       *
       * Puoli solua on 7,5 kaarisekuntia eli 230 m päiväntasaajalla:
       * reunimmainen solukeskipiste on yhä ikkunan sisällä, joten
       * yhtään aineistoa ei jää pois, mutta pyyntö ei enää koske
       * rajaa. Reunus (2 solua) kattaa tämän moninkertaisesti.
       */
      if (Math.abs(p1 / 15 - Math.round(p1 / 15)) < 1e-9) p1 -= 0.5 / 240;
      ulos.push([p0, p1]);
    }
    return ulos;
  };
  /*
   * PÄIVÄMÄÄRÄRAJA. Arkin itäisin laattasarake ulottuu yli 180 °:n
   * (mitattu: sarake 166 pyysi ikkunaa 180…181,275), ja siitä
   * `lahdeLaatta` muodosti nimen `E180`. Sellaista tiedostoa ei ole:
   * NOAA:n 15°-laatat ovat W180…E165, eli 180 °E ON W180:n
   * länsireuna. Vastaus oli HTTP 404 ja 154 laattaa jäi hakematta.
   *
   * Väli katkaistaan siis ±180:ssä, ja rajan yli mennyt osa haetaan
   * kierrettynä (−360). `siirto` kuljettaa kierron mukana, jotta
   * liimaus osaa panna palan takaisin oikeaan kohtaan globaalissa
   * hilassa — pala itse palaa aina normalisoituna −180…180:een.
   */
  const kaarra = (a, b) => {
    const osat = [];
    let alku = a;
    while (alku < b) {
      const seuraava = Math.floor(alku / 360 + 0.5) * 360 + 180;
      const loppu = Math.min(b, seuraava);
      if (loppu > alku) {
        const keski = (alku + loppu) / 2;
        osat.push([alku, loppu, Math.round((keski + 180) / 360 - 0.5) * 360]);
      }
      alku = loppu;
      if (osat.length > 8) throw new Error('päivämääräraja: liikaa osia');
    }
    return osat;
  };

  const lonR = [];
  for (const [a, b, siirto] of kaarra(lon0, lon1)) {
    for (const [p0, p1] of rajat(a - siirto, b - siirto)) lonR.push([p0, p1, siirto]);
  }
  const latR = rajat(lat0, lat1).map(([a, b]) => [a, b, 0]);

  const palat = [];
  for (const [la0, la1] of latR) {
    for (const [lo0, lo1, siirto] of lonR) {
      // eslint-disable-next-line no-await-in-loop
  ...
  for (const p of palat) {
    const px = hilaX(p.lon[0] + (p.siirto ?? 0)); const py = hilaY(p.lat[0]);
  ...
  for (const p of palat) {
    const dx = hilaX(p.lon[0] + (p.siirto ?? 0)) - x0;
