 ...
 */
/*
 * ZOOMATUT NÄKYMÄT (Fable 20.9.2026, omistajan kaappaukset marseille-
 * nimiot-reunassa-v1980.webp ja biskajanlahti-pilat-v1980.webp):
 * ongelma näkyy myös kohdemaan sisällä zoomattuna, ei vain
 * saapumisnäkymässä. `kohde` = { x, y, osuus }: kamera ajetaan laudan
 * kaupungin päälle näkyvällä leveydellä osuus × saapumisnäkymän leveys
 * (pallolauta.kamera.ajaKamera, sama ajo kuin pelin omilla
 * siirroilla). Ilman kohdetta mitataan saapumisnäkymä kuten ennen.
 */
const MITTAA = (kohde = null) => sivu.evaluate(async (kohde) => {
  const l = window.matkakirja.ui.pallolauta;
  await l.saavu({ kesto: 0 });
  await new Promise((v) => setTimeout(v, 300));
  if (kohde) {
    const leveys = (l.kamera.kameranTila()?.leveys ?? 300) * kohde.osuus;
    await l.kamera.ajaKamera({ x: kohde.x, y: kohde.y, leveys }, { kesto: 0 });
    await new Promise((v) => setTimeout(v, 300));
  }
  l.ladoHeti();
  await new Promise((v) => setTimeout(v, 300));
...
const RAJA_PX = 4;
/*
 * Zoomiaskeleet kohdemaan sisällä: enintään kolme laudan kaupunkia per
 * maa, kaksi zoomia kullekin (puolet ja neljännes saapumisnäkymän
 * leveydestä). `--ei-zoomia` mittaa vain saapumisnäkymän.
 */
const ZOOMIT = argv.includes('--ei-zoomia') ? [] : [0.5, 0.25];
const ZOOMIKAUPUNKEJA = 3;
...
for (const iso of MAAT) {
  ...
  try {
    ...
    for (const nakyma of NAKYMAT) {
      ...
      const kohteet = [null];
      for (const k of kaupungitMaittain[iso].slice(0, ZOOMIKAUPUNKEJA)) {
        for (const osuus of ZOOMIT) kohteet.push({ x: k.x, y: k.y, osuus, nimi: k.name ?? k.id });
      }
      for (const kohde of kohteet) {
        const nakymanNimi = kohde ? `${nakyma.nimi}/${kohde.nimi}×${kohde.osuus}` : nakyma.nimi;
        const tulos = await MITTAA(kohde);
        const yli = tulos.kaikki.filter((m) => m.maxYli > RAJA_PX);
        for (const m of yli) {
          loydokset.push({
            maa: m.iso || iso, testattuMaa: iso, nakyma: nakymanNimi, ...m,
          });
        }

        console.log(`${iso} ${nakymanNimi.padEnd(28)} nimiöitä ${String(tulos.kaikki.length).padStart(3)}`
