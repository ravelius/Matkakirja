...
//     <ne_10m_admin_0_countries.geojson> <ulos.geojson>
//
...
function janaOnRaja(maski, [ax, ay], [bx, by], d = ETAISYYS) {
...
export function rajajaksot(rengas, maski, d = ETAISYYS) {
  const n = rengas.length - 1; // viimeinen = ensimmäinen
  if (n < 3) return [];
  const luokka = new Array(n);
  for (let i = 0; i < n; i += 1) luokka[i] = janaOnRaja(maski, rengas[i], rengas[i + 1], d);
...
export function maamaaRajat(maapolygonit, ne) {
  ...
  const features = maat.map(({ iso, renkaat }) => {
    ...
    for (const r of renkaat) {
      koko += pituus(r);
      for (const v of rajajaksot(r, maski)) { viivat.push(v); raja += pituus(v); }
    }
...
if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  ...
  const { features, tilasto } = maamaaRajat(j, JSON.parse(readFileSync(nePolku, 'utf8')));
  writeFileSync(ulos, JSON.stringify({
    type: 'FeatureCollection',
    lahde: `${j.lahde}; luokittelu: admin-0-maamaski ${MASKI}°, koepisteet ±${ETAISYYS}°`,
