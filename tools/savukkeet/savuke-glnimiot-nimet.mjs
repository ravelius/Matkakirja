 ...
 *   1. runko syntyi ja sovitin jakoi: gl + css2d = ladonnan nimet;
 *   2. fonttien latauduttua KAIKKI nimet ovat rungolla — DOMissa ei
 *      yhtään .pallolauta-nimi-elementtiä (poistuvat pois lukien);
 *   3. paikka: kaappaus kerros näkyvissä ja piilossa — jokaisen nimen
 *      laatikossa (datumin osumalaatikko, pinnanRuutupiste) on mustetta
 *      ja laatikoiden ulkopuolella eroja on enintään 0,5 % ruudusta;
 *   4. liike: panorointi rakentaa kerrosta enintään ladontojen tahdissa
 *      (LADONNAN_TAHTI_MS) ja kehys pysyy alle 0,5 ms;
 *   5. ei sivuvirheitä.
 *
 *   PLAYWRIGHT_JS=<playwright/index.js> node tools/savukkeet/savuke-glnimiot-nimet.mjs
...
const lueTila = () => sivu.evaluate(() => {
  ...
  return {
    ...
    sovitin: s?.tila?.() ?? null,
    nimia: l.nimet?.tulos?.().nimia ?? null,
    dom: [...document.querySelectorAll('.pallolauta-nimi')].filter((e) => !e.classList.contains('pallolauta-poistuu')).length,
...
await sivu.waitForFunction(() => {
  const l = window.matkakirja.ui.pallolauta; const t = l.glSovitin?.()?.tila?.();
  return t && t.jakoja > 0 && t.css2d === 0 && t.gl > 0;
...
const odotetut = await sivu.evaluate(() => {
  ...
  const kam = p.camera();
  // Ruutupiste kirjaston omalla projektiolla levossa (sama kuin CSS2D:n paikka).
  const laatikot = ui.pallolauta.nimet.osumat().map((o) => {
    const l = o.laatikko({ x: 0, y: 0 });
    const r = p.getScreenCoords(o.lat, o.lng, 0);
...
const kaappausGL3 = await sivu.screenshot({ type: 'png' });
{
  ...
  // laatikoiden ulkopuolella erot ovat vähäiset (≤ 0,5 % ruudusta: laatta voi tarkentua).
  const osuudet = laatikot.map((l, j) => +(peitto[j] / Math.max(1, (l.x1 - l.x0) * (l.y1 - l.y0))).toFixed(3));
  tulos.erot = { sisalla, ulkona, osuus: +osuus.toFixed(3), laatikoita: laatikot.length, osuudet, laatikot };
  vartio('jokainen nimi omassa laatikossaan, ulkona vähän', laatikot.length > 0 && osuudet.every((o) => o >= 0.04) && ulkona <= 0.005 * W * H,
    `laatikoiden peitot ${JSON.stringify(osuudet)}, ulkona ${ulkona} px (${(100 * ulkona / (W * H)).toFixed(2)} %)`);
}
...
writeFileSync(join(ULOS, `glnimiot-nimet-${NAKYMA}.json`), JSON.stringify(tulos, null, 1));
await ctx.close();
await selain.close(); palvelin.close();
const kaatui = tulos.vartiot.filter((v) => !v.ok).length;
console.log(`glnimiot-nimet ${NAKYMA}: ${tulos.vartiot.length - kaatui}/${tulos.vartiot.length}`);
