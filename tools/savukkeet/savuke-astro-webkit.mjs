...
async function avaaPeli(s) {
  /*
   * SIVUNLATAUKSEN AIKAKATKO 60 s, EI PLAYWRIGHTIN OLETUS 30 s.
   * Chromiumin toinen kotelo ei ehtinyt 30 s:aan (edellisen erän
   * havainto 1), ja tämä riittää siihen.
   *
   * WEBKITILLE SE EI RIITÄ (mitattu 18.9.2026, Mac Studio): `load` ei
   * laukea 60 s:ssakaan, eikä vika ole hitaudessa vaan siinä, että
   * WebKit pitää estetyt ulkoverkon pyynnöt vireillä (ks. moduulin
   * otsikko, VERKKO). Reitityksen `abort` ei sulje niitä WebKitin
   * `load`-laskurista. Oikea korjaus on `waitUntil: 'domcontentloaded'`
   * + oma odotus linssin valmiuteen, ja se on oma eränsä — WebKit-luvut
   * jäivät siksi tästäkin erästä saamatta.
   */
  await s.goto(`http://127.0.0.1:${PORTTI}/index.html?lauta=pallo&pallodiag=1${LISAPARAMIT}`, { waitUntil: 'load', timeout: 60000 });
