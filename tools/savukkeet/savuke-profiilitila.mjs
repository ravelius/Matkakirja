 ...
 *   T1 osoitelippu ?koe=profiili,syotekello → "koe 3/5 Yhteinen kello · profiili pN · vNNNN"
 *   T2 valikon tallennus (syotetouch + kytkin) ilman lippua → "koe 2/5 Kosketus suoraan"
 *   T3 valinta vaihdetaan kesken istunnon → "(seuraavassa latauksessa: koe 1/5 Oletus)"
 *   T4 ei sivuvirheitä
...
try {
  {
    ...
    tieto('T1 rivi', r);
    vaadi('T1 osoitelippu näkyy ylimpänä', /^koe 3\/5 Yhteinen kello · profiili p\d+ · v\d+$/.test(r) && r.endsWith(versio), r);
  ...
  }
  {
    ...
    const r = await ylinRivi(sivu);
    tieto('T2 rivi', r);
    vaadi('T2 valikon valinta näkyy', r.startsWith('koe 2/5 Kosketus suoraan · profiili p'), r);
    await sivu.evaluate(() => localStorage.setItem('matkakirja-piirtokoe', 'normaali'));
    const r3 = await ylinRivi(sivu, 'seuraavassa');
    tieto('T3 rivi', r3);
    vaadi('T3 vaihto näkyy seuraavana', r3.startsWith('koe 2/5 Kosketus suoraan (seuraavassa latauksessa: koe 1/5 Oletus)'), r3);
