...
## 8. Sovitus mainiin (merge 13.9.2026, erä 1b+2 → erä 3)

Erä 3 lähti commitista `801b43ee`; mainiin ehti sen jälkeen erä 1b+2
(v1844/v1845), ja `js/pallolauta/lauta.js` konfliktoi kahdesta syystä.

1. **Nimitörmäys.** Erä 1b+2 toi tilamuuttujan `let maanLaatikko = null`
   (uloszoomauksen katko) ja erä 3 samannimisen async-funktion. Erän 3
   funktio nimettiin **`haeMaanLaatikko(iso)`**; kutsupaikat
   (`saapumisrajaus`, `laatikkoNyt`) ja tests/maakartuutsi.test.mjs:n
   vartiot päivitettiin. Tilamuuttuja `maanLaatikko` pysyi ennallaan.
2. **`saavu` yhdistettiin.** Se lukee nyt erän 3 **paneelilla
   laajennetun** laatikon (`saapumislaatikko()`) ja asettaa sen myös
   erän 2 uloszoomauksen katoksi ennen kamera-ajoa:
   `const bbox = await saapumislaatikko(); maanLaatikko = bbox;
   tahdistaZoomirajat(); return kamera.kotiin({ kesto, bbox });`
   Sama vaihto tehtiin `paivita`n väritaso/zoomikatto-haaraan, jotta
   maanvaihdon kautta tuleva katto on sama laatikko.

**Zoomikaton semantiikka:** katto = SAAPUMISLAATIKKO ×
`ULOSZOOMAUKSEN_KERROIN` (1,15), ei enää maan oma laatikko × 1,15.
Saapumisnäkymä ja uloin sallittu näkymä ovat siis sama laatikko, ja
maapaneeli on näkyvissä myös uloimmalla zoomilla — juuri se, mitä luku
6 kohta 3 pyysi erältä 2. Laajennus tehdään vain kun paneeli on
kartalla (`maapaneeliKartassa()`), joten nurkkatilassa katto on
entinen maan laatikko.

**Värilaatasto ei liiku.** Väritason rajaus (maan laatikko × 1,15,
poltettu leikkuri ja feidaus) lasketaan generaattorissa ISO-koodin
mukaan (`tools/generoi-laattapyramidi.mjs`), ja pelissä väritaso
valitaan pelkällä ISO:lla (`js/laattapyramidi.js asetaVaritasonMaa`).
Se ei lue `maanLaatikko`-muuttujaa lainkaan, joten paneelilaajennus ei
tarvinnut erillistä maan omaa laatikkoa.

**Portit sovituksen jälkeen:** `npm test` 3308/3308 (0 fail),
kaksoisavaimet ja niputus kunnossa, `tarkista-savukkeet` kunnossa,
`build-standalone` syntyy, ei konfliktimerkkejä.
`savuke-maapaneeli` **9/9 vihreä** yhdistetyssä puussa.
`savuke-varilaatat-pallo` **ohittui kokonaan** (kaikki 11 väitettä):
se vaatii pilottilaataston (`--laatat <kansio>`), jota ei ole tässä
työpuussa, ja lopettaa ilman sitä OHITUS-viestiin. Sen uloszoomausväite
V7 mittaa löyhästi (`0 < maxKorkeus < 2,4` ja kamera pysähtyy siihen),
ei kerrointa tiukasti maan laatikosta, joten laajennettu laatikko ei
riko sitä eikä väitettä tarvinnut muuttaa; `savuke-maapaneeli`n
mittaamat uloimman zoomin korkeudet (0,627 @ 390 px, 0,344 @ 1400 px)
ovat selvästi V7:n rajan alla.
