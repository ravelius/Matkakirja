# Maakuntien erä 3: GBR pulu-kysymykset (22.9.2026)

Neljälle Ison-Britannian alueelle (England, Scotland, Wales, Northern Ireland)
kirjoitetut `pulu`-kentän kysymys-vastaus-parit. Malli: `js/linssit/ihmisen-matka-kysymykset.js`
(esikirjoitetut, faktapohjaiset vastaukset), mutta kevyempi muoto `{ q, a }`.
Kysymykset eivät toista England/Scotland/Wales/Northern Ireland -alueiden
olemassa olevia `lyhyt`- ja `pitka`-tekstejä (js/packs/maakunnat-luonnehdinnat.js),
vaan täydentävät niitä uusilla, Wikipediasta tarkistettavilla faktoilla.
...
Valmiit alueet: 4/4 (England, Scotland, Wales, Northern Ireland).
