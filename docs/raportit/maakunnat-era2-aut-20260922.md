# Maakunnat erä 2: Itävalta (AUT) — pitka + kuva (Sisältökirjuri 22.9.2026)

Tilaus: `pitka`- ja `kuva`-kentät AUT:n 9 nykyiselle osavaltiolle
`js/packs/maakunnat-luonnehdinnat.js`-tiedoston `lyhyt`-kenttien rinnalle.
Tätä tiedostoa EI ole muokattu — koodimuutos jää Fablen/Opuksen
tehtäväksi tämän raportin pohjalta. Kuvat on ladattu ja tarkistettu,
osoitteet alla ovat lopullisia (sha256 laskettu ladatusta tiedostosta).
...
*(Huom: ei löytynyt sopivalla lisenssillä kuvaa itse Seebühne-näyttämöstä; kuva näyttää Bodenjärven ja Vorarlbergin rannan, jolla festivaali pidetään.)*
...
9/9 aluetta tehty (pitka + kuva molemmat kaikille). Ei jäänyt kesken.
Ladatut kuvatiedostot ovat `/Users/samireivinen/Matkakirja-nostot-kuvat/maakunnat/AUT/kuvat/`
(sha256 laskettu näistä tiedostoista, hash näkyy lopullisessa osoitteessa).
`js/packs/maakunnat-luonnehdinnat.js`-tiedostoa ei ole koskettu — Fable/Opus
lisää `pitka`- ja `kuva`-kentät tästä raportista hyväksynnän jälkeen.
