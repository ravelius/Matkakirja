...
| 176 | P0: "Pallo on vieläkin rikki" (1.0.25, Kreikka loitonnettuna Eurooppaan, kuva kaappaukset/omistaja-20260926/loydos176-pallo-rikki-kreikka-125.png): pohjan laatat puuttuvat laajalta alueelta — Pohjois-Eurooppa ja Itämeri pelkkää pergamenttia, ladatut laatat näkyvät teräväreunaisina "paperiarkkeina" (laattarajat), yksi irrallinen laatta vasemmassa yläkulmassa; vain Keski-/Etelä-Euroopan laatat piirtyvät. 163:n toistuma loitonnuksessa (ei saapumislento). Lisäksi Kreikan lippu seisoo Mustanmeren rannalla (Bulgaria/Romania), ei Kreikassa — tarkista lippuankkuri GRC. Kreikan päällä 175:n harmaat möykyt. | Natiiviseppä (laattojen lataus/vartija loitonnuksessa, laitetoisto 1.0.25 TF-buildilla iPad Pro 13; loki) + Karttaseppä (GRC lippuankkuri) |
| 177 | P1: "Uusi peli" ei tyhjennä kaikkia tietoja, eikä uusi peli ala oikein (omistaja, 1.0.25). Selvitettävä mitä jää (kuljettu reitti, löydökset, maakuntien herääminen, salaisuudet, kukkaro 300£ 1/80, musiikin tila, esilatauksen tila) ja miksi aloitus ei etene (saapumislento, Pulu, ensimmäinen kaupunki). Web on malli: uusi peli = tallennuksen täysi nollaus + aloitusjakso alusta. | Pelikoodari (PeliOhjain-tallennus, nollauslista) + Natiivi-UI (Uusi peli -polku, vahvistus) + Laitetestaaja (kierrokseen: uusi peli kesken pelin) |
| 170c | Omistaja vahvisti: sovelluksen täyden uudelleenkäynnistyksen jälkeen Kreikan maakuntakuvat NÄKYVÄT (Attika: Akropolis, kuvat kaappaukset/omistaja-20260926/loydos170-attika-uudelleenkaynnistys-a/b.png). Juurisyy siis: taustapäivityksen delta (v17x → v181) asentuu, mutta käynnissä oleva sovellus ei lataa uusia kuvia/pikkukuvadataa ennen uudelleenkäynnistystä (kuvavälimuisti tai luonnehdintadata jää vanhaksi). Korjaus: deltan asennuksen jälkeen luonnehdinta-/kuvadatan uudelleenlataus ja kuvavälimuistin mitätöinti ilman uudelleenkäynnistystä (kuten kaupunkilehdet). Kuvissa myös 176 (laatat puuttuvat) ja 173 (valikko). | Siirtoseppä (delta → "sisältö vaihtui" -tapahtuma) + Natiivi-UI (maakuntakortin datan ja kuvien uudelleenlataus tapahtumasta) |
