...
- **Klo 07.2x BUILD 19 LEIKATTU juna/b13 fdc47632** (Fable). EI pohja-26:ta (omistajan kuvaparipäätös → build 20). Juna kiinni kierroksen ajaksi. Käännös juna-ajo.sh:lla käynnissä → Laitetestaajan simulaattorit; SHA Laitetestaajalle + Fablelle. Kierroksen painopisteet (Fable → Laitetestaaja): lepopiirto saapumisen jälkeen, kylmäkäynnistys tilannekuvalla, kohta 1 ilman verkkoa, taustapäivitys v145 → uusin säilyy. PASS → master-merge (puu = käännöksen puu) → SHA Fablelle + Julkaisijalle.
- **BUILD 19 KÄÄNNETTY 343ca803** (vahti 07.32; = master 43a6347c + juna fdc47632, puu adcf61d8), asennettu Laitetestaajalle; SHA Laitetestaajalle + Fablelle 08.1x. PASS → git merge --no-ff juna/b13 Matkakirja-proto-masteriin (puu = adcf61d8) → SHA Fablelle + Julkaisijalle.
- ELÄVÄ KARTTA KOHTA 2 (build 20): natiiviseppa/nosto-muste **77bcaed4** (juna fdc47632:n päällä, EI junassa – juna kiinni build 19 -kierroksen ajan): NostoKerros.Muste delegaatti (luokka/löydetty/näkyy), Nosto.Luokka/Loydetty, PaivitaMuste, salaisuus pois Naytettavat-listasta. Natiivi-UI: merkin koko/peitto/nimi/hehku NostotKartallaan + delegaatin kytkentä; Linssiseppä: jäljen ulkoasu + löydön animaatio (Laikka.shader on 3D → sovittava UITK-toteutus).
- **Pohja 26 HYVÄKSYTTY** (omistaja 08.1x, Fable): natiiviseppa/pohja-26 c4c46662 build 20 -junaan heti build 19 -kierroksen (PASS + master-merge) jälkeen. Build 20 -jono: pohja-26, nosto-muste 77bcaed4, lepo-suspend 651ad27b (A/B ensin).
- Build 20 -merge-pyyntö: natiivi-ui/nosto-muste **fed603b9** (sis. natiiviseppa/nosto-muste 77bcaed4 + linssiseppa/muste-jaljet fcbdb97f; todennettu 3ec54edd: nostot himmeinä jälkinä, löytö täyttää merkin + Attiki herää, lepo 244/244 kaikki 0; kuvat lokit/natiivi-ui-b20-muste/). → Build 20 -jono: pohja-26 c4c46662, natiivi-ui/nosto-muste fed603b9, lepo-suspend (A/B).
- b19-kierros (Laitetestaaja b117edc23, docs/raportit/savukierros-b19-20260926.md): aloitusverho sim 5,4–6,1 s (laitemittaus), LEPOPIIRTO EI TOTEUDU VERKOTTOMANA (30–114/150; pyydetty pallo lepo -syy; epäily laatat < 100 % tai Laattapalvelin kiireinen) → build 20 -korjaus; Attikan pysyvä väri puuttuu (data: karttavaloista puuttuu maakunta-kenttä; nosto-muste build 20); huntu paljastus komento.txt-kanavalla. PASS/FAIL kysytty.
- **BUILD 19 PASS** (b117edc23) → **proto-master 41dd79c7** (puu adcf61d8 = käännös 343ca803). SHA Fablelle + Julkaisijalle (TF 1.0.19). Juna auki build 20:lle. Fablen build 20 -jono: A) kylmän aloitusverhon laitemittaus (raja 5,3 s), B) heränneen maakunnan pysyvä väri (data: maakunta-kenttä; nosto-muste fed603b9), C) lepopiirto verkottomana (ui offline -rivi), D) salaisuuskortin laukaisija Laitetestaajalle; sitten pohja-26 c4c46662 + nosto-muste fed603b9.
- **Build 20 -juna juna/b13 427e1a1a**: + natiiviseppa/pohja-26 c4c46662 (omistaja hyväksyi) + natiivi-ui/nosto-muste fed603b9 (testit 278/284/316). C/D/B-tarkistus Natiivi-UI:lle (ui offline -rivi likainen, salaisuuskortin laukaisu, Attikin pysyvä täyttö + maakunta-avain). A = laitemittaus kierroksella.
- 08.26 TF 1.0.19 -vienti alkoi (ajo 36220773751, 41dd79c7); vientilippu päällä → poista kun valmis.
- Natiivi-UI: C) natiivi-ui/offline-lepo 7d6f7bc3 (offline-pilleri/kaupunkilappu seisoo levossa Joutosykkeellä) → merge-pyyntö mittauksen kanssa. D) salaisuuskortin laukaisu välitetty Laitetestaajalle. B) Attiki pysyy (m2-loyto.png), GRC maakunta-avain OK; FRA:n maakunta-kenttä kysytty Pelikoodarilta.
- B-RATKAISU (Fable 08.3x): maakunta-kenttä ON tuotannossa (v151: 2 782/2 963 karttavaloa, Pelikoodari). Linssisepän FRA 0/0/13 johtui todennäköisesti vanhaan (v145) jääneestä paketista simulaattorissa (1.0.18:n siivousvika, korjattu build 19:ssä) → ei koodikorjausta; tarkistus: kaytossa-versio kehittäjäriviltä.
...
- juna/b13 **63984096**: + pelikoodari/musiikki-vaihe1 4218bbd8 (musiikkisuunnitelma vaihe 1, todennettu lokit/musiikki-v1/ajo1) + pelikoodari/kuljettu-reitti e5ee31dc (Linssisepän rajapinta, ei näkyvää). 153 tulee Pelikoodarin (tai seuraajan) merge-pyyntönä.
