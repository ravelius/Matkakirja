...
W, H = 1024, 1366
import os
POHJA = os.environ.get('POHJA', 'kuvat/kall-paiva.png'); ULOS = os.environ.get('ULOS', 'kuvat/kall-hamara2.png'); HAMARA = os.environ.get('HAMARA', '1') == '1'
paiva = Image.open(POHJA).convert('RGB').resize((1024, 1366))
...
if 'topo' in POHJA:
    paikka(paiva, (0, 1190), (0, 1270), (200, 96), sade=22)     # TOPOGRAFIALINSSI-merkki: sama ranta ylempää
else:
...
tulos = ImageChops.add(ImageChops.add(ImageChops.add(hamara, halo2), halo1), valo) if HAMARA else paiva
# Kortin alue (pelin nimikortti) peitetään selitelaatikolla HTML:ssä.
tulos.save(ULOS); valo.save('kuvat/valot.png')
...
for m in sorted({r['iso3'] for r in R}):
    ...
    koko = 'pieni' if a is None or alue else 'iso' if a >= 2e6 else 'keski' if a >= 5e5 else 'pieni'
