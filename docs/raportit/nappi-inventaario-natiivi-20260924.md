...
| Julisterivi (poster-galleria) | **SELVITETTY (Natiivi-UI, 25.9.):** rivin kosketusalue (`mk-laukku__julisteet`, kerros 40) on ehjä nykyisessä masterissa — testattu b12j:llä (bcc46ef) iPhone 17:llä, osui koordinaattiin (180, 480). Laitetestaajan 1572C658-yrityksissä (24.9. klo 23.5x, 5 epäonnistunutta tap-yritystä) oli todennäköisesti vanhempi asennettu build ja/tai 402 pt -näytöllä osuma rivin ohi. **Natiivi-UI löysi ja korjasi samalla oikean vian**: galleria ja laukun vedokset olivat TYHJIÄ, koska julisteen kuva-URL kaksinkertaistui (`…/julisteet/https://…` → 404) — korjaus `natiivi-ui/juliste-url` (de8852b), tulossa build 12:een. | PASS (nappi), kuvat olivat VIKA → korjattu de8852b | EI (rakennevertailu, ei pikselimitattu) |
...
| Isoisän linssi 1873 | **TESTATTU** (`linssi isoisa-1873`, 24.9. klo 23.4x): avautuu, näyttää 1873-aikakauden kartan vanhoilla maa-/aluenimillä (esim. "RANSKA" vanhoilla maakuntarajoilla) ja "ISOISÄN LINSSI 1873" -otsikkochipin, sulkunapin ja ☰-valikon kuten muutkin linssit. Web-kuvaparia ei otettu tälle kierrokselle (ei WEB ON MALLI -mitattu). | SAMA (rakenteeltaan, ei mitattu) | EI (rakennevertailu, ei pikselimitattu — kuva natiivi-isoisa1873-iphone.png) |
