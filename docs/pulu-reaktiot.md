...
- **Pilotti:** Marseille (js/packs/fokusvirta-marseille.js, 6 reaktiota,
  ...
  6 ankkuria + luonnollinen loppu 6/6 julkisessa pelissä: Marseille T
  (v1759/v1762).
...
Tilat päivitetty 11.9.2026 Codexin K/T-raporttien (v1757–v1762) mukaan.
...
| `matka.lento.lahto` | Kartta | js/ui.js:19434 `doFly` (naksu/kalvo js/ui.js:19452–19456), js/game.js:2746 `actionFly` | Napautus → `flight-active`, kaari kartalle | `jannitys` (0,5) | Kerran per lento | Ele ENNEN kalvoa; kalvon aikana `flight-active` estää kaiken (js/livia-eleet.js:155) | Lentoasento | Katse ylös | Kalvo vain maailmankartalla; mantereella suora hyppy | K/T (v1759) | Kaupungissa: Lennä → valitse kohde + julkinen v1759 |
...
| `laukku.auki` | Matkalaukku | js/ui.js:16951 `openPassport` (showModal js/ui.js:16960) | Laukku aukeaa | `utelias` (0,4) | Kerran per avaus | Modaali: pulu on kartalla laukun alla — ele vain, jos pulu näkyy (js/livia-eleet.js:123 estää `dialog[open]`) | Lepo | — | **Modaali estää nykyisin kaiken** — ks. puutelista T1 | K/T (v1757) | Napauta yläpalkin pilleriä |
| `laukku.kiinni` | Kartta | js/ui.js:16977 `suljeLaukku` | Laukku sulkeutuu, kartta palaa | `lammin` (0,3) | Kerran per sulku | Väistyy, jos kartalla on kesken oleva kupla | Lepo | Katse karttaan | — | K/T (v1757) | Sulje laukku |
...
| `lehti.aihe.kaupunki` | Aihesivu, id `kaupunki` (190 sivua) | js/lehti.js:553 → js/maalehti.js:859 | otsikko piirretty | `ylpea` (0,4) | kerran per sivu, aiheen vaihtuessa | ei kesken luennan | lepo | esittelevä (`present`) | — | K/T (v1759) | Lontoo → sivu "Lontoo pintaa syvemmältä" |
| `lehti.aihe.historia` | id `historia` (82+89) | js/maalehti.js:859 | otsikko | `miettiva` (0,45); sota/kuolema → **`vakava` (0,5)** | kerran per sivu | `livianAiheEle` pudottaa jo synkän tekstin `listen`-eleeseen | lepo | lasit | — | K (v1759) | Krakova → Historia |
| `lehti.aihe.luonto` | id `luonto`, `vuoret`, `aavikko` | js/maalehti.js:859 | otsikko | `utelias` (0,45) | kerran per sivu | — | lepo | pään kallistus | — | K (v1759) | Islanti → Luonto |
| `lehti.aihe.ruoka` | id `ruoka` | js/maalehti.js:859 | otsikko | `lammin` (0,5); leipä/pulla → `ilo` (0,6) | kerran per sivu | pullasana voittaa muun sävyn (`livianAiheEle`) | lepo | — | — | K/T (v1759) | Ranska → Ruokaa ja tapoja |
| `lehti.aihe.kulttuuri` | id `musiikki`, `taide`, `kuvataide`, `nykytaide` | js/maalehti.js:859 | otsikko | `lammin` (0,45) | kerran per sivu | — | lepo | — | — | K (v1759) | Itävalta → Musiikki |
| `lehti.aihe.tekniikka` | id `tiede`, `keksinnot` | js/maalehti.js:859 | otsikko | `miettiva` (0,5) | kerran per sivu | — | lepo | lasit | — | K (v1759) | Britannia → Keksinnöt |
| `lehti.aihe.kauppa` | id `kauppa`, `menovinkit` | js/maalehti.js:859 (+ listamalli :925) | otsikko | `ylpea` (0,4) | kerran per sivu | — | lepo | asiantuntija | — | K (v1759) | Italia → Menovinkit |
| `lehti.aihe.sana` | id `kirjallisuus`, `kielet`, `sadut`, `kirjat` | js/maalehti.js:859 | otsikko | `miettiva` (0,4) | kerran per sivu | — | lepo | lasit | — | K (v1759) | Tanska → Sadut |
| `lehti.aihe.merenkulku` | id `meri`, `laivat` | js/maalehti.js:859 | otsikko | `utelias` (0,5) | kerran per sivu | — | lepo | katse ylös | — | K (v1759) | Portugali → Meri |
| `lehti.aihe.urheilu` | id `urheilu` | js/maalehti.js:859 | otsikko | `ilo` (0,55) | kerran per sivu | — | lepo | virne | — | K (v1759) | Brasilia → Urheilu |
| `lehti.aihe.elain` | id `elaimet` | js/maalehti.js:859 | otsikko | `ilo` (0,55); lintujutussa 0,65 | kerran per sivu | lintu/kyyhky nostaa voimakkuutta (pulu itse aiheena) | lepo | — | — | K (v1759) | Australia → Eläimet |
| `lehti.aihe.huuto` | Skandaalinosto aihesivulla | js/fokusnosto-symbolit.js:637 (symboli), js/maalehti.js:859 | otsikko | `hammastys` (0,55) | kerran per sivu | — | lepo | — | — | K (v1759) | Kaupunki, jolla skandaalisivu |
| `lehti.aihe.silma` | Havainto-/katsaussivu | js/maalehti.js:859 | otsikko | `utelias` (0,5) | kerran per sivu | — | lepo | katse ylös | — | K (v1759) | — |
| `lehti.aihe.ihme` | Kadonnut ihme lehtisivulla | js/fokuskohteet.js:4452 (symboli) | otsikko | `hammastys` (0,6) | kerran per sivu | ≥ 0,6 sallittu: ihme on suuri hetki | lepo | — | — | K (v1759) | Aleksandria → ihmesivu |
| `lehti.aihe.hetki` | id `hetki-*` (33 sivua) | js/packs/kulttuuri-kategoriat.js | otsikko | `jannitys` (0,5) | kerran per sivu | `vakava`, jos hetki on tuho (Lontoon palo, muuri) | lepo | lasit | — | K (v1759) | Lontoo → "Lontoon palo 1666" |
...
| `chat.odotus.pitka` | Sama | js/pollo.js:1434 mietintärivi, 1386 `MIETINNAN_JATKOVIIVE` 6000 | 6 s → rivi vaihtuu `pitkat`-repliikkiin | `hammentynyt` (0,3) | kerran, ei toistoa | ei saa keskeyttää odotuselettä | — | — | — | K (v1759) | Throttle 3G, kysy pitkä kysymys |
...
| `aarre.kysymys.viimeinenYritys` | sivu 1, varoitusrivi | js/visa.js `VIIMEISEN_YRITYKSEN_VAROITUS`, `viimeinenYritys` | varoitus nousee napin ylle | `jannitys` (0,6) | vain kun `yritysLuku.nyt >= kaikki` (2/2) | voittaa tervehdyksen tagin — sävy on nyt varoitus | perustila | — | modaali | K/T (v1759) | tests/kohtaamistesti.test.mjs + julkinen v1759 |
...
| `aarre.rosvo.voitto` | tuloskortti | js/visa.js `answerDuelUi` (`duel.right`) | "Voitit rosvon — saalis N puntaa!" | `ilo` (0,65) | kerran per kaksintaistelu | alle löytökynnyksen (0,7): saalis ei ole aarre | perustila | — | modaali | K/T (v1759) | tests/rules.test.mjs + julkinen v1759 |
| `aarre.rosvo.tappio` | tuloskortti | js/visa.js `answerDuelUi` / `timeUp` (duel-haara) | "Rosvo vei rahat" | `vakava` (0,55) | kerran | menetys on vakava, ei hämmentävä | perustila | — | modaali | K/T (v1759) | tests/rules.test.mjs + julkinen v1759 |
...
| `peli.linssi.avautui` | ilmoituskupla | js/game.js r. 1030–1038 `emit('aid','Uusi linssi', …)` | uusi katselutapa aukeaa | `ilo` (0,65) | kerran per linssi | kokemuksella ansaittu, ei laatan alta. Sama hetki kuin `laukku.uusi-linssi` (A) | perustila | — | ilmoituskerros | K (v1759) | tests/matkalaukun-linssit.test.mjs |
...
| `linssi.ihmisen-matka.jakso.afrikka` | Valot | kertomus.js:81 | jakso alkaa | `hammastys` (0,5) — maanosa syttyy | jakson alussa | — | — | — | — | K | jakso 2 |
| `linssi.ihmisen-matka.jakso.jebel-irhoud` | Marokko | kertomus.js:90–92 | jakso alkaa | `lammin` (0,4) | jakson alussa | — | — | — | — | M/K | jakso 3 |
| `linssi.ihmisen-matka.jakso.siirtyma-afrikka` | Itä-Afrikka | kertomus.js:104 | jakso alkaa | `utelias` (0,35) | jakson alussa | — | — | — | — | K | jakso 4 |
| `linssi.ihmisen-matka.jakso.omo` | Jokilaakso | kertomus.js:113 | jakso alkaa | `miettiva` (0,4) — "meille ikuisuus, heille kotiseutu" | jakson alussa | — | — | — | — | K | jakso 5 |
...
| `linssi.ihmisen-matka.jakso.levantti` | Karmelvuori | kertomus.js:136 | jakso alkaa | `vakava` (0,4) — asutus katosi | jakson alussa | — | — | — | — | K | jakso 7 |
| `linssi.ihmisen-matka.jakso.arabia` | Arabia | kertomus.js:151 | jakso alkaa; Blombos syttyy hiljaisena nostona | `hammastys` (0,5) | jakson alussa, ei hiljaisen noston kohdalla | hiljainen nosto ei saa omaa tagia | — | — | — | K | jakso 8 |
| `linssi.ihmisen-matka.jakso.intian-rannat` | Sumatra | kertomus.js:179 | jakso alkaa | — (hiljaisuus antaa tilaa Australialle) | — | — | — | — | — | K | jakso 9 |
| `linssi.ihmisen-matka.jakso.australia` | Rannikkomeri | kertomus.js:191–193 | jakso alkaa | `hammastys` **0,68** (korjattu sisältöön v1767/v1768) | jakson alussa | huippukohta; toimituslinjan katto 0,7 | — | — | — | M/K (ylitys, korjattava sisällössä) | jakso 10 |
...
| `linssi.ihmisen-matka.jakso.napapiiri` | Arktinen | kertomus.js:220 | jakso alkaa | `vakava` (0,45) — "silti he jäivät" | jakson alussa | — | — | — | — | K | jakso 12 |
...
| `linssi.ihmisen-matka.jakso.white-sands` | Tundra | kertomus.js:245 | jakso alkaa | `lammin` (0,42) — lapsen ja aikuisen jalanjäljet | jakson alussa | — | — | — | — | K | jakso 14 |
| `linssi.ihmisen-matka.jakso.chile` | Metsäsade | kertomus.js:258 | jakso alkaa | `ylpea` (0,5) — "pisin kävelymatka" | jakson alussa | — | — | — | — | K | jakso 15 |
| `linssi.ihmisen-matka.jakso.aikahyppy` | Kelaus | kertomus.js:271 | kello kelaa taaksepäin | — (tekninen liike) | — | — | — | — | — | K | jakso 16 |
| `linssi.ihmisen-matka.jakso.eurooppa` | Bulgaria | kertomus.js:280 | jakso alkaa | `vakava` (0,45) — neandertalilaiset katosivat | jakson alussa | synkkä sävy: ei virnettä | — | — | — | K | jakso 17 |
| `linssi.ihmisen-matka.jakso.chauvet` | Luola | kertomus.js:293–295 | jakso alkaa | `rakkaus` (0,48) | jakson alussa | — | — | — | — | M/K | jakso 18 |
| `linssi.ihmisen-matka.jakso.meri` | Avomeri | kertomus.js:307 | jakso alkaa | `utelias` (0,45) — purjehdus tähtien avulla | jakson alussa | — | — | — | — | K | jakso 19 |
| `linssi.ihmisen-matka.jakso.uusi-seelanti` | Rantalinnut | kertomus.js:318 | jakso alkaa | `ylpea` (0,5) — viimeinen suuri maa | jakson alussa | ei saa syödä `loppu`-jakson lämpöä | — | — | — | K | jakso 20 |
