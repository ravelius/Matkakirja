...
test('jokainen puheVoima-tasolla alkava luenta merkitsee itsensä puhujaksi', () => {
  ...
  for (const polku of ['../js/luenta.js', '../js/linssipuhe.js', '../js/ui.js']) {
    ...
    const osumat = rivit
      /*
       * NELJÄ ALOITUSMUOTOA 14.9.2026 alkaen. iOS ei tottele elementin
       * omaa volumea (js/musiikkivahvistin.js): js/luenta.js kirjoittaa
       * tason luentaSoittimen kautta, ja linssiluenta kirjoittaa
       * tasonsa oman soittimensa kautta ja hihkaisu vahvistimeen.
       * Portti tunnistaa kaikki muodot, jottei uusi reitti pääse
       * puhujakirjanpidon ohi.
       */
      .map((rivi, i) => (/^\s*(?:audio\.volume = puheVoima\(\);|const audio = luentaSoitin\(.*puheVoima\(\)\);|const audio = linssiluennanSoitin\(.*puheVoima\(\)\);|else audio\.volume = puheVoima\(\);)\s*$/.test(rivi) ? i : -1))
      .filter((i) => i >= 0);
