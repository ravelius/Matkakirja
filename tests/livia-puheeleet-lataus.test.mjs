...
test('validaattori vaatii kortin, näkyvän tekstin sekä mp3:n SHA:n ja tavut', async () => {
  ...
  assert.equal((await tarkistaLivianPilottiData({ ...data, revision: 'vanha-revisio' },
    { kaupunki: 'marseille', teksti, aani })).ok, false);
  assert.equal((await tarkistaLivianPilottiData({ ...data, aani: { ...data.aani, sha256: '0'.repeat(64) } },
    { kaupunki: 'marseille', teksti, aani })).ok, false);
