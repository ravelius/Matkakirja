...
import { LIVIA_PIX_ELEET, LIVIA_PIX_RUUDUT, LIVIA_PIX_W, LIVIA_PIX_H, LIVIA_PIX_LEFT, livianNayttamo, livianPikseliAsento, livianPikselit } from '../js/livia-pikselit.js';
...
test('pikselisarjan kaikki 51 liikettä pysyvät lavalla, ja lepo varaa vain 22 × 22',()=>{
...
test('lento muuttuu pisteeksi, kävely ylittää napin ja lepotila on 16 × 16',()=>{
 ...
 assert.ok(Math.max(...xy.map(p=>p[0]))-Math.min(...xy.map(p=>p[0]))<16);
 assert.ok(Math.max(...xy.map(p=>p[1]))-Math.min(...xy.map(p=>p[1]))<16);
 ...
 for(const right of [0,7,25,120]){
  ...
  if(right>0)assert.ok(walk.some(r=>r.slice(38).some(Boolean)),'kävelee myös alkuperäisen kuvakkeen oikealla puolella');
 }
 for(const id of ['flyAway','flyBack','clumsyLand','glassCrash','walkRight','walkBack'])for(let i=0;i<=100;i++){
  const p=livianNayttamo(livianPikseliAsento(id,i/100),{right:7});assert.equal(p.length,76);assert.ok(p.every(r=>r.length===45&&r.every(v=>v>=0&&v<=3)));
 }
});
