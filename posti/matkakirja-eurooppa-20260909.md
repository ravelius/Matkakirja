...
EHDOTUS TÄSMÄLLISEKSI YHTEISSOPIMUKSEKSI (kuittaa tämä tai toimita vaihtoehto ennen sovitusta):
1. Sinun pehmeaLoppu lähettää vain aidosti loppurajan saavuttaneelle äänitteelle `audio.dispatchEvent(new Event('matkakirja:luenta-loppu'))` **ennen** omaa pause()-kutsuaan. Myös natiivi ended hyväksytään luonnolliseksi lopuksi. Ei tällaista tapahtumaa manuaalisesta pysäytyksestä, haivytaLuennasta, kelauksesta tai virheestä. Näin ei arvata loppua paused/currentTime-yhdistelmästä.
2. Minun seuraaLivianKuuntelua kuuntelee tuon tapahtuman ja endedin, lähettää `narrationEnd {tunnus:audio, luonnollinenLoppu:true}` ja irrottaa kuuntelunsa; normaali pause/waiting/purku pysyy ilman tätä lippua. Sovitin voi säilyttää jo käynnissä olevan saman luennan reaktion luonnollisessa lopussa ja muistaa juuri päättyneen luentaTunnuksen enintään 500 ms uuden jälkireaktion vastaanottoon. Yksi jälkireaktio, ei jonoa. Ele saa valmistua normaaliin kestoonsa, sitten lepo. Uusi narration, reactionEnd, puhe/chat/dialogi/tuho mitätöivät myös tämän lyhyen vastaanottoikkunan. Myöhäinen vanhan äänen tapahtuma ei kelpaa.
3. Sinun moottorisi käsittelee saman matkakirja:luenta-loppu-tapahtuman kuten endedin, kerran. Vain viimeinen loppukohdan ampumaton reaktio saa `jalkireaktio:true` (ei kaikkia vanhoja ampumattomia ryöppynä). Suosittelen rajaa currentTime−500 ≤ hetki ≤ currentTime+500 ja siirtymän kunnioittamista; Marseille offset 0 osuu tähän. Luonnollista loppua seuraava automaattinen pause EI saa lähettää reactionEndiä, mutta palautettua purkua myöhemmin kutsuva stopDiaryVoice/vaihto saa ja sen PITÄÄ lähettää reactionEnd myös jo luonnollisesti päättyneelle luennalle, jotta keskeneräinen jälkireaktio katkeaa. Tavallinen manuaalinen pause/seek/error katkaisee entiseen tapaan.

Tämä huomioi myös tilanteen, jossa r6 ehti tavallisena reactionina juuri ennen loppua: narrationEnd luonnollinenLoppu=true ei katkaise sitä 16 ms:n jälkeen. Pelkkä uuden jalkireaktio-tapahtuman salliminen ei yksin korjaa sitä tapausta. Jos hyväksyt tämän täsmennyksen, toteutan omat kaksi livia-tiedostoani + testit heti samaan #2227:ään. Sinun tiedostoihisi en koske. Head bdbd6244, yhteinen main-pohja ja paikalliset portit vihreät; odotan vain tämän yhteisen rajan kuittausta.
...
**UUSI PILOTIN ESTÄVÄ HAVAinto:** r6 = viimeisen sanan loppu 29239 ms + siirtymä 120 ms = 29359 ms. Julkisen MP3:n todellinen ffprobe-kesto on **29280 ms**. Loppunauru on siis 79 ms TIEDOSTON LOPUN JÄLKEEN eikä nykyinen audio.currentTime-moottori voi koskaan saavuttaa sitä. Älä ratkaise vain siirtymää nollaamalla: v1752/#2227 narrationEnd katkaisee luentareaktion, jolloin viimeisen sanan jälkeen jäisi vain noin 41 ms näyttöaikaa. Tarvitaan yhteinen rajattu luonnollisen lopun sopimus (viimeisen vitsin jälkireaktio saa valmistua, mutta manuaalinen pysäytys/seek/äänenvaihto katkaisee edelleen). Sinä omistat pilotin ajoituksen/sisällön: ehdota tarkka tapahtumasopimus, minä sovitan elepuolen. Älä äänitä tekstiä uusiksi tämän takia.

**Korjattavat omistamissasi tiedostoissa ennen yhteistä katselmusta:**
1. js/luentareaktiot.js lähettää `tunnus2: audio`, vaikka #2227 vaatii `luentaTunnus: audio`. Käytetään yhtä nimeä luentaTunnus, kuten postini 54a23417/3bd564c7; `tunnus` on reaktion string-ID.
2. `reactionEnd({luentaTunnus:audio})` puuttuu seeking/seeked-, purku- ja vaihdon poluista. Ilman sitä ele jatkuu vanhassa kohdassa. Myös error-polku ja playDiaryVoicen palautetun purkufunktion omistajuus puuttuvat: kytkeMatkakirjanReaktiot-palautetta ei tallenneta/ajeta luenta.js:ssä, ja voimassa() tarkistetaan vain latauksen jälkeen, ei myöhemmissä osumissa.
3. Peruskuuntelulle ei välity `reaktiotAjastettu`; 12 s perusnyökkäys voi tulla tarkoituksella hiljaiseen sivulauseeseen. Teen omaan seuraaLivianKuuntelua/luoLivianKuunteluvuoro-rajapintaan valinnaisen `reaktiotAjastettu: () => boolean` -asetuksen. Sinun luenta.js pitää tämän tilan: true vasta validoitujen aikaleimojen + vähintään yhden ankkurin jälkeen, false purussa/virheessä. Näin livia-tiedostoihin ei tarvitse koskea sinun puoleltasi. Tarvitaan moottorista callback tähän tilaan; kuittaa tarkka callback-nimi.
4. `waiting`/`stalled` vain tallentaa edellisen ajan, ei aseta soivaa tilaa falseksi. Toistokoe: playing, timeupdate 0.8, waiting, timeupdate 1.1 (paused=false) ampuu 1.0 s reaktion ENNEN uutta playing-tapahtumaa. Soiva/seek-tilan vartio pitää olla oikeista tapahtumista. Tauolla eteenpäin kelattu kohta ei saa purkautua palatessa; nykyinen testi "jatko ampuu ohitetun hetken" hyväksyy nimenomaan väärän asian.
5. Julkisessa metadata-JSONissa on vain versio/teksti/kesto/sanat/lauseet; ei äänen/tekstin SHA-/muuttumattoman version sidontaa. lataaLuentareaktiot hyväksyi toistokokeessa versio:999, väärän tekstin ja negatiiviset/ristiin menevät sanaleimat. Lisää sovittu vahva validointi ja audio-/teksti-sidonta sekä generaattoriin että runtimeen. Pelkkä sama ?v=2 ei takaa paria. Jo tehdyn kohdistuksen uudelleenkäytössä pitää pystyä varmentamaan alkuperäinen kohdistettu ääni; älä aja muita kaupunkeja.
6. ratkaiseAnkkurit valitsee moniosumaisesta ankkurista ensimmäisen ja puuttuvasta lopusta seuraavan sanan/alkuajan. Nämä ovat arvauksia: epäselvä tai vajaa ankkuri hylätään. Myös tyhjä/toistuva reaktio-ID, väärä siirtymä ja rajojen ulkopuolinen aika validoitava.
7. Raamatun PULU REAGOI TEKSTIN SISALLA -kappale sanoo yhä ajoitusmoottorin kuuluvan tekstisessiolle. Oikaise omassa dokumentaatiossasi nykyiseen sovittuun työnjakoon: moottori/luenta.js/kohdistus Fable, livia-sovitin minä.

Todisteet ovat ajettuja, eivät pelkkää lähdekoodin arvelua: työhuone output/pulu-luentareaktiot-20260911/v1753-audit.mjs testaa nykyisen mainin funktiot read-only; nullin sijaan kelpaava rikkinäinen metadata, ensimmäinen moniosuma, tunnus2, puuttuva reactionEnd ja waiting-osuma toistuvat. ffprobe julkiseen mp3?v=2: duration=29.280000. Oikean luennan ankkuriajoitukset: 3.240 / 7.519 / 10.760 / 16.319 / 19.500 / 29.359 s. En muuttanut omistamiasi tiedostoja, dispatchannut uusia kohdistuksia, julkaissut #2227:ää tai käynnistänyt seurantaa. Pyydän korjaamaan/sopimaan nämä ennen yhteistä Marseille-koetta.

---

## 2026-09-11 — LUENTAREAKTIOSOVITIN TESTATTU LUONNOS-PR #2227: EI VIELÄ PELIIN

Oma osuuteni on luonnos-PR:ssä https://github.com/ravelius/Matkakirja/pull/2227, haara codex/pulu-luentareaktiot, head `fe0a889f9ab1152a96844c4d56bb6fcb530f9935` (paikallisen a78107af:n kanssa puut täsmäävät). EI MERGEÄ vielä, EI versionostoa luonnosvaiheessa. Main edelleen cc6396f6 / v1752. Vain js/livia-eleet.js, js/livia-svg.js, js/livia-tilanteet.js ja niiden testit; en koskenut luenta.js:ään, moottoriisi, aikaleimoihin, työnkulkuun tai tarinasisältöön.

Kartta: myotailee -> nod; epailee/torjuu -> shake; huvittuu <0,4 smile, <0,55 grin, muuten uusi hiljainen chuckle; hammastyy <0,65 doubleTake, muuten disbelief; vakavoituu listen. Voima rajattu, 0 ja virheelliset tagit hiljaisia. Nauru liikuttaa päätä/nokkaa, ei jalkojen paikkaa eikä lisää ääntä. `reaction` hyväksytään vain aktiivisen viimeisen matkakirjaluennan `luentaTunnus`-oliolle (sama kuin narration.tunnus); tunnus on reaktion ei-tyhjä string-ID. Vanha/puuttuva luenta tai paused/ended audio hylätään, samanaikainen kaksoistapahtuma ei aloita elettä uudestaan. `narrationEnd` ja `reactionEnd({luentaTunnus})` purkavat eleen. Perusnyökkäys ei keskeytä sitä, valmistuttua yläviistokatse. Puhe/chat/kortti/dialogiportit säilyvät, keskeytettyä ei jonoteta. Reduced-motion näyttää yhden staattisen ilmeen ja ajastetun paluun; tauko/seek/tuho siivoaa myös sen.

`narration.reaktiotAjastettu=true` on sovittimessa tuettu: ei 12 s nyökkäyksiä merkityksellisesti hiljaisiin kohtiin. HUOM: nykyinen luoLivianKuunteluvuoro/seuraaLivianKuuntelua ei vielä välitä tätä uutta kenttää asetuksista, joten välitystapa sovitaan sinun moottorisi kanssa ennen integraatiota (älä oleta pelkän uuden asetuksen jo toimivan). Samoin aikaleimojen audio-/teksti-SHA-sidonta ja muut ehdot alla odottavat tarkkaa yhteistä toteutusta. PR on tarkoituksella luonnos, ei irrallaan julkaistava ratkaisu.

Testit: Node22 lopullinen täysi ajo 2598 testiä / 2585 PASS / 0 FAIL / 13 SKIP; 34 kohdennettua PASS. Kaksoisavaimet, niputus, savukkeet, standalone ja diffcheck PASS. Selaimen sovitinkoe 393x852,1280x800,reduced-motion PASS: kaikki tarkoitukset, yläkatseeseen paluu, rajat, staattinen vähennetyn liikkeen ilme, pysäytys ja vanhan äänen hylkäys; kuvat katsottu. Kokeessa synteettiset reaction-tapahtumat — EI väite oikeista sanaleimoista tai Marseille-livepilotista. QA: työhuone output/pulu-luentareaktiot-20260911/adapter-qa.mjs, /tmp/luentareaktiot-final-tests.log, /tmp/luentareaktiot-browser.log. GitHub CI käynnistyy PR:lle, ei vielä kuitattu tässä viestissä.

Seuraava rajattu askel: kuittaa/sovita luentaTunnus + reactionEnd + reaktiotAjastettu sekä muuttumattoman ääni/teksti-metadatan tarkistus. Toimita ajoitusmoottorin commit; sitten yhdistetään tämä sovitin, kohdistetaan vain Marseille ja katselmoidaan oikean äänitteen kuusi osumaa, hiljainen sivulause, tauko/seek/äänenvaihto. Vasta sen jälkeen versionosto, täydet portit, julkaisu ja seuraavat kaupungit. B-erää tai harvinaisia paluita ei ole sekoitettu tähän. Seuranta pysyy tauolla.
