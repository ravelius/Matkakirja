...
/** Naulauksen työsäikeen polku sivun juuresta (ks. js/pallovektorit-tyo.js). */
export const NAULAUKSEN_TYOSAIE_POLKU = 'js/pallovektorit-tyo.js';
 ...
 */
/*
 * RANNIKON NAULAUS asuu omassa puhtaassa moduulissaan
 * (js/pallovektorit-naulaus.js), jotta sama laskenta ajaa työsäikeessä;
 * nimet viedään tästä edelleen, joten kutsujat ja testit eivät muutu.
 */
export {
...
export function luoPallovektorit({ pallo, kotelo, ikkuna = globalThis, reitit }) {
  ...
  /*
   * NAULAUKSEN TYÖSÄIE (sulavuus E1, 21.9.2026; ks. js/pallovektorit-tyo.js).
   * Käynnistyy kerran; käynnistymis- tai ajovirhe pudottaa pääsäikeen
   * polkuun (naulaus synkronisesti kuten ennen) — tulos on sama, vain
   * säie eri. `?naulaussaie=0` ottaa säikeen pois vertailua varten.
   */
  let tyosaie = null;
  const saieSallittu = (() => {
    try { return new URLSearchParams(ikkuna.location?.search ?? '').get('naulaussaie') !== '0'; } catch { return true; }
  })();
  if (saieSallittu && typeof ikkuna.Worker === 'function' && ikkuna.document?.baseURI) {
    try {
      tyosaie = new ikkuna.Worker(new URL(NAULAUKSEN_TYOSAIE_POLKU, ikkuna.document.baseURI), { type: 'module' });
      const varapolku = (syy) => {
        if (!tyosaie) return;
        console.warn(`Naulauksen työsäie ei toiminut (${syy}); naulaus pääsäikeessä.`);
        try { tyosaie.terminate(); } catch { /* jo poissa */ }
        tyosaie = null;
        mittarit.naulaussaie = false;
        if (korostus.odottaa) { korostus.odottaa = null; rakennaKorostus(); }
      };
      tyosaie.addEventListener('error', (e) => varapolku(e.message ?? 'error'));
      tyosaie.addEventListener('messageerror', () => varapolku('messageerror'));
      tyosaie.addEventListener('message', (e) => {
        const v = e.data ?? {};
        if (purettu) return;
        if (v.tyyppi === 'virhe') { varapolku(v.viesti); return; }
        if (v.tyyppi !== 'naulattu' || v.pyynto !== korostus.odottaa?.pyynto) return;
        const { haivella } = korostus.odottaa;
        korostus.odottaa = null;
        viimeisteleKorostus(v, haivella);
      });
      mittarit.naulaussaie = true;
    } catch {
      tyosaie = null;
      mittarit.naulaussaie = false;
    }
  ...
  /** Näkyvien rannikkosolujen tunnukset työsäikeelle. */
  function rannikkoSoluIdt() {
    const ulos = [];
    for (const [id, s2] of solut) {
      if (s2.laji === 'rannikko' && s2.viivat?.length && nakyvat.has(id)) ulos.push(id);
    }
    return ulos;
  }
  ...
  function rakennaKorostus(haivella = false) {
    ...
    if (!renkaat?.length) { vapautaKorostus(); return; }
    /*
    ...
    if (!nakyvatRenkaat.length) { vapautaKorostus(); return; }
    const naulaa = naulattava();
    korostus.naulattuHetki = nyt();
    korostus.rannikkoja = naulaa ? rannikkoViivoja() : 0;
    // Porras kirjataan heti: odotuksen aikana kehys ei saa tilata uutta.
    korostus.harvennus = harvennus;
    /*
     * NAULAUS TYÖSÄIKEESSÄ (sulavuus E1, 21.9.2026): pääsäie lähettää
     * renkaat ja näkyvien rannikkosolujen tunnukset, ja VANHA korostus
     * pysyy ruudulla kunnes vastaus tulee — ei nykäystä eikä välähdystä.
     * Myöhästynyt vastaus (uudempi pyyntö lähti) hylätään.
     */
    if (tyosaie && naulaa) {
      const pyynto = korostus.pyynto + 1;
      korostus.pyynto = pyynto;
      korostus.odottaa = { pyynto, haivella };
      try {
        tyosaie.postMessage({
          tyyppi: 'naulaa', pyynto, renkaat: nakyvatRenkaat, solut: rannikkoSoluIdt(), asetukset: {},
        });
        mittarit.naulauksia = (mittarit.naulauksia ?? 0) + 1;
        return;
      } catch {
        korostus.odottaa = null;
        // Viesti ei mennyt: sama laskenta pääsäikeessä alla.
      }
    }
    const rannikot = naulaa ? rannikkoviivat() : [];
    const naulaus = naulaaKorostus(nakyvatRenkaat, rannikot);
    viimeisteleKorostus(naulaus, haivella);
  }

  /** Naulatuista viivoista geometria; vanha olio pois vasta nyt. */
  function viimeisteleKorostus(naulaus, haivella) {
    if (purettu || !materiaalit || !luokat || !kolmi?.juuri) return;
    const porrasNyt = korostus.harvennus;
    vapautaKorostus();
    korostus.harvennus = porrasNyt;
  ...
  function lataa(laji, k, avain) {
    ...
    s.lupaus = ikkuna.fetch(osoite)
      ...
      .then((puskuri) => {
        ...
        if (tyosaie && laji === 'rannikko') {
          try { tyosaie.postMessage({ tyyppi: 'solu', id, viivat: s.viivat }); } catch { /* säie kuoli */ }
        }
        const t0 = nyt();
        rakenna(s);
  ...
  function vapauta(s) {
    vapautaOlio(s);
    s.viivat = null;
    if (tyosaie && s.laji === 'rannikko') {
      try { tyosaie.postMessage({ tyyppi: 'poista', id: `${s.laji}/l${s.k}/${s.avain}` }); } catch { /* säie kuoli */ }
    }
  }
