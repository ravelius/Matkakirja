...
import { livianSvgAsento, luoLivianSvg, livianEleenVoima } from './livia-svg.js';
import {LIVIAN_UUDET_PELIELEET,livianUusiPelitila,livianUusiPelikuva} from './livia-uudet-versiot.js';
...
import { kuunteleLivianKasvopuhetta, kuunteleLivianKasvopuheenElinkaarta, livianKasvopuheenTila } from './livia-puhetila.js';
...
/* Lukee kummankin karttakameran yhteistä tilaa. Ei muuta kameraa eikä
 * korvaa sen tapahtumakäsittelijöitä. Myös inertialiuku havaitaan. */
function seuraaLivianKarttaliiketta({haeKamera,aktiivinen,muuttui}) {
 let ajastin=0,kuollut=false,edellinen=null,kamera=null,liikkuu=false,viimeLiike=0;
 function nollaa(){edellinen=null;kamera=null;if(liikkuu){liikkuu=false;muuttui(false);}}
 function tarkista(){
  if(kuollut)return;
  if(!aktiivinen()){nollaa();return;}
  const uusi=haeKamera(),tila=uusi?.kameranTila?.();
  if(!tila||![tila.x,tila.y,tila.skaala].every(Number.isFinite)||tila.skaala<=0){nollaa();return;}
  const nyt=performance.now();
  if(kamera!==uusi){nollaa();kamera=uusi;}
  if(edellinen){
   const siirtyma=Math.hypot(tila.x-edellinen.x,tila.y-edellinen.y)*tila.skaala;
   const zoomi=Math.abs(Math.log(tila.skaala/edellinen.skaala));
   if(siirtyma>.2||zoomi>.0002){liikkuu=true;viimeLiike=nyt;muuttui(true);}
   else if(liikkuu&&nyt-viimeLiike>=500){liikkuu=false;muuttui(false);}
  }
  edellinen={x:tila.x,y:tila.y,skaala:tila.skaala};
 }
 function kierros(){tarkista();if(!kuollut)ajastin=setTimeout(kierros,100);}
 // Ensimmäinen havainto on vertailupiste, ei uusi liike.
 kierros();
 return {nollaa,tuhoa(){kuollut=true;clearTimeout(ajastin);nollaa();}};
}
...
export function asennaLivianKasvot(pollo) {
 ...
 let kasvot;try{kasvot=luoLivianSvg(canvas,{kuva:livianUusiPelikuva});}catch{return null;}
 ...
 const aktiivisetPuheet=new Set();
 let viimePuheTunnus=null;
 ...
 function puheEleenOsuus(ele){
  ...
  if(!Number.isFinite(alku)||!Number.isFinite(loppu)||!Number.isFinite(hetki)||loppu<=alku)return null;
  const mediaOsuus=ele.id==='cityExplain'?(hetki-alku)/(loppu-alku):(hetki-alku)/ele.duration;
  return Math.max(0,Math.min(1,mediaOsuus));
 }
 function piirraNyt(nyt){
  let s=nykyinen?livianUusiPelitila(nykyinen.id,osuus,{voimakkuus:nykyinen.voimakkuus,
 ...
 function askel(nyt){
  ...
  if(nykyinen){
    const mediaOsuus=puheEleenOsuus(nykyinen);
    osuus=mediaOsuus??Math.min(1,(nykyinen.fromProgress||0)+(nyt-alkoi)/nykyinen.duration);iskut();
    if(osuus>=1){
 ...
 function toista(id,asetukset={}){
  ...
  const ele=LIVIAN_UUDET_PELIELEET.find(e=>e.id===id);if(!ele||kuollut)return false;
  if(kohtausPiilossa()&&!['startFlight','trailer'].includes(asetukset.omistaja))return false;
  katkaise();lepoTila=null;
  if(!nakyy()||(vahenna?.matches&&!['reaction','speechCue','bunGranted'].includes(asetukset.omistaja))){piirraNyt(performance.now());return false;}
  nykyinen={...ele,voimakkuus:livianEleenVoima(id),...asetukset};
  // Koreografia valitaan cuen käynnistyessä. Kesken pitkän eleen tuleva
  // 1x→2x ei saa vaihtaa lintua sivuasennosta paikalleen yhdessä ruudussa.
  // Tauolta/seekiltä alkava uusi elinkaari käyttää silloin voimassa olevaa nopeutta.
  if(nykyinen.omistaja==='speechCue'&&!Number.isFinite(nykyinen.cuePlaybackRate))nykyinen.cuePlaybackRate=Math.max(.25,Number(nykyinen.puheTunnus?.playbackRate)||1);
  osuus=puheEleenOsuus(nykyinen)??(asetukset.fromProgress||0);aaniIndeksi=0;alkoi=performance.now();viimeEle=alkoi;viimePiirto=-Infinity;
  if(vahenna?.matches){
   ...
   if(nykyinen.omistaja!=='speechCue')reaktioAjastin=setTimeout(()=>{katkaise();piirraNyt(performance.now());},nykyinen.duration);
   return true;
  }
 ...
 function tilanne(laji,tiedot={}){
  ...
  if(laji==='narration'){
   if(nykyinen?.omistaja==='speechCue'){katkaise();lepoTila=null;piirraNyt(performance.now());}
  ...
  if(laji==='speechCueEnd'){
   const samaCue=typeof tiedot.tunnus==='string'&&tiedot.tunnus===nykyinen?.tunnus;
   const samaPuhe=Boolean(tiedot.puheTunnus)&&tiedot.puheTunnus===nykyinen?.puheTunnus;
   // Cue-loppu ei ole koko puhevuoron lopetus. Saman äänitteen edellisen
   // cuen myöhäinen loppu ei saa katkaista jo alkanutta seuraavaa cuea.
   // Puhevuoron oma loppu kulkee kasvopuheen elinkaarisignaalista.
   if(nykyinen?.omistaja!=='speechCue'||!samaCue||(tiedot.puheTunnus&&!samaPuhe))return false;
   katkaise();lepoTila=null;piirraNyt(performance.now());kaynnista();return true;
  }
  if(laji==='speechCue'){
   const r=livianPuheeleenTiedot(tiedot),nyt=performance.now();
   if(!r||typeof tiedot.tunnus!=='string'||!tiedot.tunnus||!tiedot.puheTunnus
    ||tiedot.puheTunnus!==viimePuheTunnus||!aktiivisetPuheet.has(tiedot.puheTunnus)
    ||!puhe||!nakyy()||pollo.auki||luennat.size||nostoTila?.onkoAuki()||odotusrivi||odotukset.size
    ||nykyinen?.group==='Liike'||['card','bunGranted'].includes(nykyinen?.omistaja)||lepoTila?.flight||lepoTila?.walk)return false;
 ...
 function palaa(){
  const s=nykyinen?livianUusiPelitila(nykyinen.id,osuus,{voimakkuus:nykyinen.voimakkuus}):lepoTila;
 ...
 function ajasta(){
  ...
  // UI voi valmistua Pulun jälkeen. Ei kameraa eikä ylimääräistä kelloa
  // esikatselussa/testisovittimessa; varsinainen peli saa lukevan vahdin.
  if(!karttavahti&&typeof pollo.haeUi?.()?.kamera==='function')karttavahti=seuraaLivianKarttaliiketta({
   haeKamera:()=>pollo.haeUi?.()?.kamera?.(),aktiivinen:()=>!kuollut&&!doc.hidden&&!vahenna?.matches&&nappiNakyy(),muuttui:karttaLiikkui,
  });
  kello=setTimeout(()=>{
 ...
 const astronauttiVahti=new MutationObserver(()=>piirraNyt(performance.now()));
 ...
 function liikeAsetus(){
  const jatkettavaCue=nykyinen?.omistaja==='speechCue'?{...nykyinen}:null;
  karttavahti?.nollaa();
  ...
  katkaise();lepoTila=null;
  if(jatkettavaCue&&puhe&&aktiivisetPuheet.has(jatkettavaCue.puheTunnus)){
   toista(jatkettavaCue.id,jatkettavaCue);ajasta();return;
  }
  piirraNyt(performance.now());ajasta();if(!vahenna?.matches)kaynnista();
 }
 function tausta(){
  karttavahti?.nollaa();
 ...
 const irrotaPuheElinkaari=kuunteleLivianKasvopuheenElinkaarta(({tunnus,vaihe})=>{
  if(vaihe==='puhuu'){
   aktiivisetPuheet.delete(tunnus);aktiivisetPuheet.add(tunnus);viimePuheTunnus=tunnus;
   if(nykyinen?.omistaja==='speechCue'&&nykyinen.puheTunnus!==tunnus){katkaise();lepoTila=null;piirraNyt(performance.now());}
   return;
  }
  aktiivisetPuheet.delete(tunnus);viimePuheTunnus=[...aktiivisetPuheet].at(-1)??null;
  if(nykyinen?.omistaja==='speechCue'&&nykyinen.puheTunnus===tunnus){katkaise();lepoTila=null;piirraNyt(performance.now());}
 });
 const irrotaPuhe=kuunteleLivianKasvopuhetta(tilat=>{
  ...
  if(!puhe){if(nykyinen?.omistaja==='speechCue'){katkaise();lepoTila=null;}if(chatOdotus?.lahti){katkaise();lepoTila=livianSvgAsento('flyAway',1);pinta.style.opacity='0';return;}if(lepoTila?.ele==='bookStudy')lepoTila=null;piirraNyt(performance.now());if(oli)jatkaKuunteluaTaiOdotusta();
 ...
 function tuhoa(){
  karttavahti?.tuhoa();karttavahti=null;
  doc.removeEventListener('wheel',toiminta,true);doc.removeEventListener('pointermove',raahaus,true);
  if(kuollut)return;kuollut=true;irrotaTilanteet();irrotaDialogit();nostoTila?.tuhoa();chatTila?.tuhoa();tyhjennaChatOdotus();tyhjennaAlkulento();tyhjennaTraileri();katkaise();clearTimeout(kello);for(const tunnus of [...pitkatOdotukset.keys()])peruPitkaOdotus(tunnus);irrotaPuhe();irrotaPuheElinkaari();aktiivisetPuheet.clear();viimePuheTunnus=null;nappiVahti.disconnect();lehtiVahti?.disconnect();virtaVahti.disconnect();astronauttiVahti.disconnect();pinta.remove();nappi.classList.remove('livia-kasvot-valmis');
 ...
 doc.addEventListener('wheel',toiminta,{capture:true,passive:true});doc.addEventListener('pointermove',raahaus,{capture:true,passive:true});
 ...
 astronauttiVahti.observe(doc.body,{attributes:true,attributeFilter:['class']});
 ...
 return{
  ...
  tilanne,toista,palaa,tuhoa,eleet:LIVIAN_UUDET_PELIELEET,
 };
}
