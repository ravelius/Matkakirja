 ...
 *   - KAANON KERTOI ITSE KAKSI VASTAUSTA — FABLE KORJASI SEN. Livian
 *     ensimmäinen versio nimesi Chopinin (kulttuurivisan vastaus) ja
 *     sanoi vanhankaupungin rakennetun uudelleen "vanhojen kuvien
 *     mukaan" (laattakysymyksen 2 vastaus). Aallon 4C kaanonpaperissa
 *     30.8.2026 kumpikin on poissa: nimen tilalla on "sen pianistin
 *     nimen arvaat varmaan itsekin" ja menetelmän tilalla vihje "miten
 *     se onnistui, sen jäljille pääset kohta". Kumpikin vastaus jää
 *     nyt pelaajan löydettäväksi.
 *   - TÄKY `canaletto` ON SE, MIHIN VIHJE JOHTAA. Se kertoo, mitä
 *     Bellotton maalauksille tapahtui ja miksi ne kelpasivat
 *     piirustuksiksi — eli vastaa Livian "miten se onnistui"
 *     -vihjeeseen käyttämättä laattakysymyksen vastausriviä
 *     sellaisenaan.
 *
 * ── ÄÄNITE ─────────────────────────────────────────────────────────
 *
...
export const FOKUSVIRTA_VARSOVA = {
  ...
  pollo: {
     ...
     * siksi, että juuri se on korjatun kaanonin jättämä avoin vihje
     * ("miten se onnistui, sen jäljille pääset kohta") ja täky
     * `canaletto` on kokonaan siitä. Selite ei saa vastata kysymykseen,
     * jonka Livia jättää auki kuvan vieressä.
