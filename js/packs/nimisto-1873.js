...
// 20.9.2026): { teksti, luokka, lon, lat, iso, koko?, aika, kulma? }.
...
//
// AIKA (omistajan päätös 21.9.2026, haara sisalto-nimisto-aika):
...
// siirtyvät omaan Vuosi 1873 -linssiin, eivät näy pohjakartalla.
// `aika: 'pysyva'` = nimi on nykyisin käytössä alueen nimenä (esim.
// Bretagne, Normandia, Provence, Burgundi, Korsika, Toscana, Sisilia,
// Baijeri, Andalusia, Katalonia — kaikki meret ovat aina 'pysyva').
// `aika: '1873'` = poliittinen yksikkö, jota ei enää ole tai joka on
// nykyisin eri maassa/nimellä (Elsass-Lothringen, Preussin provinssit
// kuten Itä-/Länsi-Preussi ja Posen, Vanha-/Uusi-Kastilia,
// Saksin/Meiningenin/Altenburgin/Coburg-Gothan herttuakunnat...).
// Rajatapaukset on ratkaistu kirjoittajan parhaalla arviolla samaan
// tapaan kuin `koko`-kenttä — Karttaseppä/Fable voi korjata.
// Samalla korjattu: LATIUM → LAZIO (nykyinen käytössä oleva nimi;
// ks. Italia-lohkon oma huomautus) ja KANTABRIA poistettu kokonaan
// (ks. Espanja-lohkon oma huomautus).
...
export const NIMISTO_1873 = [
  ...
  // erässä). Sisilia, Sardinia ja Apulia (Puglia) ovat fi-Wikipedian
  // omia artikkelinimiä (vakiintuneet suomalaiset/latinapohjaiset
  // eksonyymit); Piemonte, Liguria, Lombardia, Veneto, Toscana,
  // Umbria, Marche, Campania, Calabria, Basilicata jäävät
  ...
  //
  // LAZIO (korjattu 21.9.2026, omistajan päätös): rivi oli alun perin
  // "LATIUM", koska fi-Wikipedian oma artikkeli alueesta on nimeltään
  // "Latium" ("Lazio" on siellä vain täsmennyssivu). Omistaja tilasi
  // silti "Lazio"-muodon, koska se on alueen nykyinen käytössä oleva
  // nimi (ks. `aika`-kenttä alla) — tämä on tietoinen poikkeus muuten
  // johdonmukaisesta fi-Wikipedia-artikkelinimi-periaatteesta.
  //
  ...
  //
  // AIKA (Fablen tilaus 21.9.2026): 'pysyva' = nimi on nykyisin
  // käytössä alueen nimenä (esim. Piemonte, Toscana, Sisilia, Lazio);
  // '1873' = 1873-erän oma poliittinen/hallinnollinen nimi, joka ei
  // ole nykyinen (Emilia — nykyinen "Emilia-Romagna" — ja Abruzzi e
  // Molise, joka jaettiin kahdeksi alueeksi 1963).
  //
  ...
  // (Sisilia, Sardinia, Apulia, Toscana, Piemonte, Lombardia, Umbria,
  // Campania, Marche, Calabria, Basilicata — tarkistettu nimimuodon
  // vakiintuneisuus 21.9.2026). Koordinaatit ovat tämän kirjoittajan
  // arvioita alueen painopisteestä, EI tietokoneellisesti
  ...
  {
    teksti: 'LAZIO', luokka: 'maakunta', lon: 12.8, lat: 41.9, iso: 'ITA', koko: 'suuri',
    aika: 'pysyva',
  },
  {
  ...
  //
  // KAKSI HARKINNANVARAISTA PÄÄTÖSTÄ (Karttaseppä/Fable, tarkistakaa):
  ...
  // 2) LEÓN on ERILLINEN nimiö Vanha-Kastiliasta (oma kuningaskunta
  //    ennen v. 1230 liittoa Kastiliaan, ja seutu tunnettiin omana
  //    kulttuurialueenaan yhä 1800-luvulla) — ei sulautettu Vanha-
  //    Kastiliaan.
  //
  // KANTABRIA (Santander) POISTETTU 21.9.2026 (omistajan päätös): oli
  // alun perin oma nimiönsä Vanha-Kastiliasta erillään, mutta koska
  // pohjakartalle jäävät vain PYSYVÄT nimet ja Kantabria oli tässä
  // erässä rajatapaus (ks. edellinen versio tästä kommentista), rivi
  // karsittiin kokonaan pois tässä tarkistuksessa yksinkertaisuuden
  // vuoksi.
  //
  // KOKOLUOKKA (koko): suuret (Galicia, Aragonia, Katalonia, Valencia,
  // Vanha-Kastilia, Uusi-Kastilia, León, Extremadura, Andalusia) ovat
  // pinta-alaltaan selvästi suurempia kuin pienet (Asturia, Baskimaa,
  // Navarra, Murcia, Baleaarit, Kanariansaaret) — arvio tämän
  // kirjoittajan, ei koneellisesti mitattu, samaan tapaan kuin Ranskan
  // erässä.
  //
  // AIKA (Fablen tilaus 21.9.2026): 'pysyva' = nimi on nykyisin
  // käytössä alueen nimenä (esim. Galicia, Aragonia, Katalonia,
  // Andalusia); '1873' = poliittinen yksikkö, jota ei enää ole tällä
  // nimellä (Vanha-/Uusi-Kastilia, León — ks. yllä).
  //
  ...
  { teksti: 'VÄLIMERI', luokka: 'meri', lon: 15.0, lat: 38.0, iso: null, aika: 'pysyva' },
  { teksti: 'ITÄMERI', luokka: 'meri', lon: 19.0, lat: 58.0, iso: null, aika: 'pysyva' },
  { teksti: 'POHJANMERI', luokka: 'meri', lon: 3.0, lat: 56.0, iso: null, aika: 'pysyva' },
  { teksti: 'NORJANMERI', luokka: 'meri', lon: 2.0, lat: 68.0, iso: null, aika: 'pysyva' },
  { teksti: 'BARENTSINMERI', luokka: 'meri', lon: 40.0, lat: 74.0, iso: null, aika: 'pysyva' },
  /* Fablen korjaus: en-Wikipedian "White Sea" on suomeksi Vienanmeri, ei Valkoinenmeri. */
  { teksti: 'VIENANMERI', luokka: 'meri', lon: 38.0, lat: 65.5, iso: null, aika: 'pysyva' },
  { teksti: 'MUSTAMERI', luokka: 'meri', lon: 34.0, lat: 43.5, iso: null, aika: 'pysyva' },
  { teksti: 'ASOVANMERI', luokka: 'meri', lon: 37.0, lat: 46.5, iso: null, aika: 'pysyva' },
  { teksti: 'EGEANMERI', luokka: 'meri', lon: 25.0, lat: 38.5, iso: null, aika: 'pysyva' },
  { teksti: 'MARMARANMERI', luokka: 'meri', lon: 28.0, lat: 40.7, iso: null, aika: 'pysyva' },
  { teksti: 'JOONIANMERI', luokka: 'meri', lon: 19.0, lat: 38.0, iso: null, aika: 'pysyva' },
  { teksti: 'TYRRHENANMERI', luokka: 'meri', lon: 12.0, lat: 40.0, iso: null, aika: 'pysyva' },
  { teksti: 'ADRIANMERI', luokka: 'meri', lon: 15.5, lat: 43.0, iso: null, aika: 'pysyva' },
  { teksti: 'LIGURIANMERI', luokka: 'meri', lon: 8.7, lat: 43.4, iso: null, aika: 'pysyva' },
  { teksti: 'KELTIENMERI', luokka: 'meri', lon: -8.0, lat: 50.0, iso: null, aika: 'pysyva' },
  { teksti: 'IRLANNINMERI', luokka: 'meri', lon: -5.0, lat: 53.5, iso: null, aika: 'pysyva' },
  { teksti: 'BISKAJANLAHTI', luokka: 'meri', lon: -4.5, lat: 45.5, iso: null, aika: 'pysyva' },
  { teksti: 'LIONINLAHTI', luokka: 'meri', lon: 4.0, lat: 42.8, iso: null, aika: 'pysyva' },
  { teksti: 'POHJANLAHTI', luokka: 'meri', lon: 20.0, lat: 63.0, iso: null, aika: 'pysyva' },
  { teksti: 'SUOMENLAHTI', luokka: 'meri', lon: 26.0, lat: 59.8, iso: null, aika: 'pysyva' },
  { teksti: 'RIIANLAHTI', luokka: 'meri', lon: 23.5, lat: 57.5, iso: null, aika: 'pysyva' },
  { teksti: 'ENGLANNIN KANAALI', luokka: 'meri', lon: -2.0, lat: 49.8, iso: null, aika: 'pysyva' },
  { teksti: 'SKAGERRAK', luokka: 'meri', lon: 8.5, lat: 57.8, iso: null, aika: 'pysyva' },
  { teksti: 'GIBRALTARINSALMI', luokka: 'meri', lon: -5.6, lat: 35.95, iso: null, aika: 'pysyva' },
  { teksti: 'BOSPORINSALMI', luokka: 'meri', lon: 29.0, lat: 41.1, iso: null, aika: 'pysyva' },
  { teksti: 'DARDANELLIT', luokka: 'meri', lon: 26.4, lat: 40.2, iso: null, aika: 'pysyva' },
  { teksti: 'JUUTINRAUMA', luokka: 'meri', lon: 12.7, lat: 55.7, iso: null, aika: 'pysyva' },
  { teksti: 'KATTEGAT', luokka: 'meri', lon: 11.5, lat: 57.0, iso: null, aika: 'pysyva' },
  { teksti: 'MESSINANSALMI', luokka: 'meri', lon: 15.6, lat: 38.2, iso: null, aika: 'pysyva' },
];
