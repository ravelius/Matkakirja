 ...
 * sama kaava kuin aallossa 3. Teksti ja luenta ovat sanasta sanaan
 * samat, joten luennan voi ajaa suoraan (generoi-luennat-tyonkulku)
 * ilman että tekstiin kosketaan.
 */
...
export const FOKUSVIRTA_LISSABON = {
  ...
  pollo: {
    /*
     * ERILLISTÄ `maadoitus`-KENTTÄÄ EI OLE, EIKÄ SE OLE UNOHDUS.
     *
     * Aallon 3 kaupungeissa Livian puheenvuoro on kahdessa kentässä:
     * `maadoitus` vastaa isoisän merkintään ja `teksti` kääntää katseen
     * herokuvaan. Lissabonissa Fablen kaanonteksti tekee molemmat itse ja
     * yhtenä puheenvuorona: se alkaa merkinnän kaakeleista ja päättyy
     * lauseeseen "Torni odottaa joessa", joka on herokuvan osoitus. Jos
     * tekstin jakaisi kahtia, sanajärjestys muuttuisi — ja kaanonia ei
     * muuteta. Moottori kestää puuttuvan maadoituksen (js/fokusvirta.js
     * lukee sen `?? null`).
     */
    teksti: 'Lissabonissa ne kaakelit ovat edelleen joka seinällä, ja niitä '
      + 'varastetaan nykyään seiniltä niin että osa kaduista on aidattu.. '
      + 'Se järistys, josta isoisäsi kirjoitti, muutti muuten koko Euroopan '
      + 'ajattelua — sen jälkeen alettiin ensimmäistä kertaa tutkia, miksi '
      + 'maa järisee, eikä vain ketä sillä rangaistiin. Torni odottaa '
      + 'joessa.',
    /*
